...
| 数据 | 静态 JSON 文件 | GitHub Actions 定期拉取并提交 |
...
- 顶部：标题 + 简介（1句话）
- 主体：三个仓库卡片，纵向排列
- 底部：Powered by GitHub Pages + 更新时间戳
...
步骤:
  1. 依次调用 3 个仓库的 GitHub Releases API
  2. 提取每个 release 的最新版本的 assets 列表
  ...
  "repos": [
    {
      ...
      "latest": {
        ...
        "assets": [
          {"name": "ccx-windows-amd64.exe", "size": 30294016, "download_url": "https://..."}
        ]
      }
    }
  ]
...
├── app.js                  # 渲染逻辑
├── data/
│   └── releases.json       # 自动更新的 release 数据
├── .github/workflows/
